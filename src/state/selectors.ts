...
export const selectFavoriteBreweryByID = (state: RootState, id: string): AppBreweryItem => {
...
export const selectBreweryByID = (state: RootState, id: string): AppBreweryItem => {
  
  if (state.FavoritesReducer.breweries.hasOwnProperty(id)) {
    return selectFavoriteBreweryByID(state, id);
  }
  else {
